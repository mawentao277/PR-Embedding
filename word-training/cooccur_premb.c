...
static const int MAX_WORD_NUM = 40;
typedef double real;
...
int sample_num = 3; // default sample num
...
int get_word(char *word, FILE *fin) {
    ...
    while(!feof(fin)) {
        ...
        word[i++] = ch;
        if(i >= MAX_WORD_LENGTH - 1) i--;   // truncate words that exceed max length
    }
    word[i] = 0;
    return 0;
}

int get_line(char *word_arr[], int *align_indexs, FILE *fin, int *post_num, int *word_idx) {
	if (feof(fin)) return -1;
	...
	int column_index = 0;
    while(1) {	
		tag = get_word(word, fin);
		if (tag == -1) return -1;
		if (tag == 2) {
			column_index += 1;
			if (column_index == 1) *post_num = *word_idx;
			if (column_index == 2) *word_idx = 0;
			continue;
		}
		if (tag == 1) return 0;
        if (column_index == 0 && *word_idx >= (int)(MAX_WORD_NUM * 0.75)) continue;
		if (*word_idx < MAX_WORD_NUM){
			if (column_index < 2){ 
                strcpy(word_arr[*word_idx], word);
			}
            //read the alignment index to generate the cross-sentence window
			else align_indexs[*word_idx] = atoi(word);
		}
		else continue;
		*word_idx += 1;
    }
	if ((*word_idx == 0) || (*post_num == 0) || (*word_idx == *post_num)) return -1;
	else if (column_index < 3) return 0;
    else return 1;
}
...
int merge_files(int num) {
    ...
    for(i = 0; i < num; i++) {
        ...
        new.id = i;
        insert(pq,new,i+1);
		if(verbose > 1) fprintf(stderr, "Merging cooccurrence files: processed %d files.\n", num);
    }
	...
	if(verbose > 1) fprintf(stderr, "Merging cooccurrence files: Pop top node and save it.\n");
	...
	if(verbose > 1) fprintf(stderr, "Merging cooccurrence files: repeatedly pop top node\n");

	int w1_id = old.word1;	
	int w2_id = old.word2;
	real merge_val = old.val;
    fwrite(&old, sizeof(CREC), 1, fout);
	if(verbose > 1) fprintf(stderr, "Merging cooccurrence files: write the file\n");
    fprintf(stderr,"\033[0GMerging cooccurrence files: processed %lld lines.\n",++counter);
    for(i=0;i<num;i++) {
		fprintf(stderr,"Removing the %dth file.\n", i);
        sprintf(filename,"%s_%04d.bin",file_head,i);
        remove(filename);
    }
	fprintf(stderr,"Finished removing all the files.\n", i);
    fprintf(stderr,"\n");
    return 0;
}
...
int get_cooccurrence() {
    int flag, x, y, fidcounter = 1, *align_index_line;
    long long a, j = 0, k, id, counter = 0, ind = 0, vocab_size, w1, w2, *lookup, *history, *wid_line;
    char format[20], filename[200], str[MAX_WORD_LENGTH], *words_line[MAX_WORD_NUM];
    FILE *fid, *foverflow;
    real *bigram_table, r;
    ...
    wid_line = malloc(sizeof(long long) * (MAX_WORD_NUM));
	align_index_line = malloc(sizeof(int) * (MAX_WORD_NUM));

	int i = 0;
	for (i=0; i<MAX_WORD_NUM; i++){
		words_line[i] = (char*)malloc(MAX_WORD_LENGTH*sizeof(char));
	}

	for (i=0; i<MAX_WORD_NUM; i++){
		align_index_line[i] = -1;
	}
	...
	int post_num=0, word_num=0;
	j=0;
    while (1) {
        if(ind >= overflow_length - window_size - sample_num) { // If overflow buffer is (almost) full, sort it and write it to temporary file
        ...
        int have_align_index = 0;
		if(j == 0){ 
			post_num = 0;
			word_num = 0;
			have_align_index = get_line(words_line, align_index_line, fid, &post_num, &word_num);
			if (have_align_index == -1) break;

			int i = 0;
			for(i = 0; i<word_num; i++){
				htmp = hashsearch(vocab_hash, words_line[i]);
				if (htmp == NULL) continue;
				wid_line[i] = htmp->id;
			}
			history[j % window_size] = wid_line[j]; // Target word is stored in circular buffer to become context word in the future
			j++;
			continue;
		}
		if(j== post_num){
			history[(j-post_num) % window_size] = wid_line[j]; // Target word is stored in circular buffer to become context word in the future
			j++;
			continue;
		}
		if(flag==-1) continue;
		...
		int window_start = 0;
		if (j < post_num) window_start = (j > window_size) ? j - window_size : 0;
		else window_start = (j > window_size+post_num) ? j-window_size:post_num;
		for (k=j-1; k >= window_start; k--){
			if (j<post_num) w1 = history[k % window_size]; // Context word (frequency rank)
			else w1 = history[(k-post_num)%window_size];
		...
		if (j<post_num) history[j % window_size] = w2; // Context word (frequency rank)
		else history[(j-post_num)%window_size] = w2;

		int span_start=0, span_end = 0, align_index=-1, index_start, index_end;
		srand(time(NULL));
		if (j < post_num){
            if (have_align_index == 1) align_index = align_index_line[j];
			int reply_num = word_num - post_num;
			if (align_index == -1) align_index = rand() % reply_num;
			index_start = align_index - (sample_num - 1)/2;
			index_end = align_index + (sample_num - 1)/2;
			if (index_start < 0) index_start = 0;
			if (index_end > reply_num - 1) index_end = reply_num-1; 
			span_start = post_num + index_start;
			span_end = post_num + index_end;
		}
		else{
			if (have_align_index == 1) align_index = align_index_line[j];
			if (align_index == -1) align_index = rand() % post_num;
			span_start = align_index - (sample_num - 1)/2;
			span_end = align_index + (sample_num - 1)/2;
			if (span_start < 0) span_start = 0;
			if (span_end > post_num - 1) span_end = post_num -1;
		}
		for (k=span_start; k < span_end + 1; k++){
			w1 = wid_line[k];
            if ( w1 < max_product/w2 ) { // Product is small enough to store in a full array
        ...
        j++;
        if((j >= MAX_WORD_NUM) || (j == word_num)) {j = 0; continue;} // Newline, reset line index (j)
    }
...
int main(int argc, char **argv) {
    ...
    if ((i = find_arg((char *)"-sample-num", argc, argv)) > 0) sample_num = atoi(argv[i + 1]);
